...
use crate::{CudaError, abi::JxrOverlapWorkAbi, plan::CudaPlaneInput};
...
fn clipped_axis(sizes: &[u32], origin: u32, extent: u32) -> Result<Vec<u32>, CudaError> {
    let end = origin
        .checked_add(extent)
...
fn absolute(offset: usize, x: u32, y: u32, stride: u32) -> Result<u32, CudaError> {
    usize::try_from(y)
        .ok()
        ...
        .ok_or_else(|| invalid("overlap sample index exceeds the CUDA ABI"))
}
...
fn scaled(value: u32, scale: u32) -> Result<u32, CudaError> {
    value
        .checked_mul(scale)
