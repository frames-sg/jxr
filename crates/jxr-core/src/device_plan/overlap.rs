...
use super::{OverlapWork, PlanError, ReconstructionPlane};
use alloc::{vec, vec::Vec};

#[derive(Debug, Default)]
pub struct OverlapSchedule {
    pub prefix: Vec<OverlapWork>,
    pub filters: Vec<OverlapWork>,
    pub suffix: Vec<OverlapWork>,
}

pub fn first_overlap_schedule(
    plane: ReconstructionPlane,
    hard_tiles: bool,
    tile_columns: &[u32],
    tile_rows: &[u32],
) -> Result<OverlapSchedule, PlanError> {
    let regions = regions(
        plane,
        hard_tiles,
        tile_columns,
...
pub fn second_overlap_schedule(
    plane: ReconstructionPlane,
    hard_tiles: bool,
    tile_columns: &[u32],
    tile_rows: &[u32],
...
fn clipped_axis(sizes: &[u32], origin: u32, extent: u32) -> Result<Vec<u32>, PlanError> {
    let end = origin
        .checked_add(extent)
...
fn absolute(offset: usize, x: u32, y: u32, stride: u32) -> Result<u32, PlanError> {
    usize::try_from(y)
        .ok()
        ...
        .ok_or_else(|| invalid("overlap sample index exceeds the device ABI"))
}

const fn work(first: u32, second: u32, kind: u32) -> OverlapWork {
    [first, second, kind, 0]
}

fn scaled(value: u32, scale: u32) -> Result<u32, PlanError> {
    value
        .checked_mul(scale)
...
mod tests {
    ...
    fn full_schedule_centres_interior_work_inside_two_sample_edges() {
        ...
        .unwrap();
        assert_eq!(schedule.filters[0][0], 2 + 2 * 16);
        assert_eq!(schedule.filters[1][0], 6 + 2 * 16);
        assert_eq!(schedule.filters[3][0], 2 + 6 * 16);
    }
}
