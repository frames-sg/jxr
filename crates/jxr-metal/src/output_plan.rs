...
) -> Result<Vec<JxrSamplePlaneAbi>, MetalError> {
    input
        .planes
        .iter()
        .map(|plane| {
            Ok(JxrSamplePlaneAbi {
                sample_offset: u32::try_from(plane.sample_offset)
                    .map_err(|_| invalid("sample plane offset exceeds the Metal ABI"))?,
...
fn surface_descriptors(plan: &MetalDecodePlan) -> Result<Vec<JxrSurfacePlaneAbi>, MetalError> {
    plan.output()
        .planes
        .iter()
        .map(|plane| {
            Ok(JxrSurfacePlaneAbi {
                byte_offset: u32::try_from(plane.byte_offset)
                    .map_err(|_| invalid("output plane offset exceeds the Metal ABI"))?,
                row_stride_bytes: u32::try_from(plane.row_stride_bytes)
                    .map_err(|_| invalid("output row stride exceeds the Metal ABI"))?,
                width: plane.width,
                height: plane.height,
...
fn output_parameters(
...
) -> Result<JxrOutputAbi, MetalError> {
    ...
    let output_plane_count = u32::try_from(surfaces.len())
        .map_err(|_| invalid("output plane count exceeds the Metal ABI"))?;
    Ok(JxrOutputAbi {
...
fn alpha_plane(samples: &[JxrSamplePlaneAbi], channel_layout: u32) -> Result<u32, MetalError> {
...
fn validate_math_policy(
    ...
    {
        return Err(MetalError::UnsupportedOutputFormat {
